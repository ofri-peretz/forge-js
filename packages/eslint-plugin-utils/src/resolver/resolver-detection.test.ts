...
describe('resolver-detection', () => {
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), 'resolver-detection-test-'));
  });
  ...
  describe('detectResolvers', () => {
    ...
    it('should detect CSS resolver when package.json path contains react', () => {
      ...
      createTempFile('react-project/package.json', JSON.stringify({ dependencies: { react: '^18.0.0' } }));
      const results = detectResolvers(reactDir);
      const cssResult = results.find(r => r.name === 'css');
      expect(cssResult?.detected).toBe(true);
      expect(cssResult?.config).toEqual({ extensions: ['.css', '.scss', '.sass'] });
    ...
    it('should not detect CSS resolver when package.json path does not contain react', () => {
      createTempFile('package.json', JSON.stringify({ dependencies: { react: '^18.0.0' } }));
      const results = detectResolvers(testDir);
      const cssResult = results.find(r => r.name === 'css');
    ...
    it('should not detect CSS resolver when package.json does not contain react', () => {
      createTempFile('package.json', JSON.stringify({ dependencies: { lodash: '^4.0.0' } }));
      const results = detectResolvers(testDir);
      const cssResult = results.find(r => r.name === 'css');
      expect(cssResult?.detected).toBe(false);
  ...
  describe('generateRecommendedConfig', () => {
    it('should return node resolver when no resolvers detected', () => {
      ...
      expect(recommendations).toContain('Using basic Node.js resolution (no bundler config detected)');
    });
    ...
    it('should generate config with detected resolvers', () => {
      ...
      const resolverConfig = settings['import/resolver'] as Record<string, unknown>;
    ...
    it('should prioritize resolvers correctly', () => {
      ...
      const typescriptIndex = configEntries.findIndex(([name]) => name === 'typescript');
      const webpackIndex = configEntries.findIndex(([name]) => name === 'webpack');
    ...
    it('should prioritize resolvers with unknown names using fallback priority (line 105)', () => {
      ...
      const mockResolvers: ResolverDetectionResult[] = [
        { name: 'typescript', detected: true, config: { alwaysTryTypes: true } },
        { name: 'unknown-resolver', detected: true, config: {} },
        { name: 'webpack', detected: true, config: { config: 'webpack.config.js' } },
      ];
  ...
  describe('migrateFromEslintImport', () => {
    it('should preserve resolver settings', () => {
      ...
      expect(suggestions).toContain('Resolver settings preserved - no changes needed');
    });
    ...
    it('should migrate import/no-cycle rule', () => {
      ...
      const rules = migrated['rules'] as Record<string, unknown>;
      expect(rules['@forge-js/llm-optimized/no-circular-dependencies']).toEqual(['error', { maxDepth: 2 }]);
    });
    ...
    it('should migrate all mapped rules', () => {
      ...
      expect(rules['@forge-js/llm-optimized/no-circular-dependencies']).toBeDefined();
      ...
      expect(rules['@forge-js/llm-optimized/no-webpack-loader-syntax']).toBeDefined();
    });
  ...
  describe('validateResolverConfig', () => {
    it('should return valid when no resolver settings found', () => {
      ...
      expect(result.warnings).toContain('No import/resolver settings found - using defaults');
    });
    ...
    it('should reject invalid format in array', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some(e => e.includes('Invalid resolver format'))).toBe(true);
    });
    ...
    it('should reject invalid resolver settings type', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Resolver settings must be a string, array, or object');
    });
