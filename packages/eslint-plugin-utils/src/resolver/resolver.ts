...
import { resolveWithExternalResolvers, ResolverSetting } from './resolver-adapter';
...
// Key: tsconfig path
const tsConfigCache = new Map<string, ReturnType<typeof createPathsMatcher> | null>();

// Cache for package.json resolution (optimization for monorepos)
const packageJsonCache = new Map<string, { exports?: Record<string, string>; main?: string } | null>();

// Resolver performance metrics (for monitoring and optimization)
const resolverMetrics = new Map<string, {
  resolveCount: number;
...
export function getResolverPerformanceMetrics(): ResolverPerformanceMetrics[] {
  return Array.from(resolverMetrics.entries()).map(([name, metrics]) => ({
    ...
    averageResolveTime: metrics.resolveCount > 0 ? metrics.totalResolveTime / metrics.resolveCount : 0,
    cacheHitRate: metrics.resolveCount > 0 ? (metrics.cacheHits / metrics.resolveCount) * 100 : 0,
    errorRate: metrics.resolveCount > 0 ? (metrics.errors / metrics.resolveCount) * 100 : 0,
 ...
 */
function trackResolverPerformance(name: string, startTime: number, success: boolean) {
...
export function resolveModule(
  ...
  try {
    ...
    if (options.resolverSettings) {
      const externalResult = resolveWithExternalResolvers(importPath, fromFile, options.resolverSettings);
      if (externalResult) {
        success = true;
    ...
    if (!resolver) {
      resolver = ResolverFactory.createResolver({
        ...
        extensions,
        conditionNames: options.conditionNames || ['import', 'require', 'node', 'default', 'types'],
