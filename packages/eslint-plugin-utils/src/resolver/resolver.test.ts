...
import { resolveModule, clearResolverCache, getResolverPerformanceMetrics } from './resolver';
...
describe('resolveModule Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    const baseTempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'resolver-test-'));
    ...
    fs.writeFileSync(path.join(projectDir, 'src', 'index.ts'), 'export * from "./utils";');
    fs.writeFileSync(path.join(projectDir, 'src', 'utils.ts'), 'export const add = (a, b) => a + b;');
    fs.writeFileSync(path.join(projectDir, 'src', 'components', 'Button.tsx'), 'export const Button = () => {};');
    fs.writeFileSync(path.join(projectDir, 'src', 'components', 'index.ts'), 'export * from "./Button";');
    
    // node_modules/react
    fs.mkdirSync(path.join(projectDir, 'node_modules'));
    fs.mkdirSync(path.join(projectDir, 'node_modules', 'react'));
    fs.writeFileSync(path.join(projectDir, 'node_modules', 'react', 'package.json'), '{"main": "index.js"}');
    fs.writeFileSync(path.join(projectDir, 'node_modules', 'react', 'index.js'), 'module.exports = {};');
  ...
  it('resolves relative paths', () => {
    ...
    expect(resolved2).toBe(path.join(tempDir, 'project', 'src', 'components', 'Button.tsx'));
  });
  ...
  it('resolves node_modules', () => {
    ...
    expect(resolved).toBe(path.join(tempDir, 'project', 'node_modules', 'react', 'index.js'));
  });
  ...
  it('should handle CSS imports when cssSupport is enabled', () => {
    ...
    const resolved = resolveModule('./styles.css', fromFile, { cssSupport: true });
  ...
  it('should handle CSS imports with different extensions', () => {
    ...
    const resolved = resolveModule('./styles.scss', fromFile, { cssSupport: true });
  ...
  it('should not resolve CSS imports when cssSupport is disabled', () => {
    ...
    const resolved = resolveModule('./styles.css', fromFile, { cssSupport: false });
  ...
  it('should handle TypeScript paths with index files', () => {
    ...
    expect(resolved).toBe(path.join(tempDir, 'project', 'src', 'components', 'index.ts'));
  });
  ...
  it('should handle TypeScript paths with extensions', () => {
    ...
    fs.writeFileSync(path.join(tempDir, 'project', 'tsconfig.json'), JSON.stringify({
      compilerOptions: {
        baseUrl: '.',
        paths: {
          '@utils': ['src/utils-dir'],
  ...
  it('should handle custom main fields', () => {
    ...
    const testPkgDir = path.join(tempDir, 'project', 'node_modules', 'test-pkg');
  ...
  it('should handle CSS import with relative path and extension', () => {
    ...
    const resolved = resolveModule('./component.css', fromFile, { cssSupport: true });
    expect(resolved).toBe(cssFile);
  });
  ...
  it('should handle CSS import resolution failure gracefully', () => {
    ...
    const resolved = resolveModule('./nonexistent.css', fromFile, { cssSupport: true });
    expect(resolved).toBeNull();
  });
  ...
  it('should handle CSS import with SASS extension', () => {
    ...
    const resolved = resolveModule('./styles.sass', fromFile, { cssSupport: true });
  ...
  it('should handle CSS import with LESS extension', () => {
    ...
    const resolved = resolveModule('./styles.less', fromFile, { cssSupport: true });
  ...
  it('should handle CSS import with STYL extension', () => {
    ...
    const resolved = resolveModule('./styles.styl', fromFile, { cssSupport: true });
  ...
  it('should handle CSS import resolution when file exists but is not CSS', () => {
    ...
    const resolved = resolveModule('./styles.js', fromFile, { cssSupport: true });
  ...
  it('should handle resolution when all resolvers fail', () => {
    ...
    const resolved = resolveModule('completely-nonexistent-module-xyz-123', fromFile);
    expect(resolved).toBeNull();
  ...
  it('should handle CSS import with relative path starting with ./', () => {
    ...
    const resolved = resolveModule('./relative.css', fromFile, { cssSupport: true });
    expect(resolved).toBe(cssFile);
  });
  ...
  it('should handle CSS import resolution when directory exists but no index file', () => {
    ...
    const resolved = resolveModule('./empty-css-dir', fromFile, { cssSupport: true });
  ...
  it('should handle CSS import with extension but file does not exist', () => {
    ...
    const resolved = resolveModule('./missing.css', fromFile, { cssSupport: true });
    expect(resolved).toBeNull();
  });
  ...
  it('should handle CSS import when isCssImport returns true for file with extension', () => {
    ...
    const resolved = resolveModule('./test.css', fromFile, { cssSupport: true });
    expect(resolved).toBe(cssFile);
  });
  ...
  it('should handle CSS import when isCssImport matches relative path with extension', () => {
    ...
    fs.mkdirSync(path.join(tempDir, 'project', 'src', 'styles'), { recursive: true });
    fs.writeFileSync(cssFile, 'body { padding: 0; }');
    
    // Test relative path with CSS extension
    const resolved = resolveModule('./styles/main.css', fromFile, { cssSupport: true });
    expect(resolved).toBe(cssFile);
  });
  ...
  it('should handle CSS import resolution when resolveCssImport returns null', () => {
    ...
    const resolved = resolveModule('external.css', fromFile, { cssSupport: true });
  ...
  it('should handle CSS import when cssSupport is false (should not enter CSS resolution)', () => {
    ...
    const resolved = resolveModule('./test.css', fromFile, { cssSupport: false });
  ...
  it('should handle CSS import resolution success with all CSS extensions', () => {
    ...
    for (const ext of extensions) {
      ...
      clearResolverCache();
      const resolved = resolveModule(`./test${ext}`, fromFile, { cssSupport: true });
      expect(resolved).toBe(cssFile);
    }
  });
  ...
  it('should handle CSS import resolution when file exists with exact path', () => {
    ...
    const resolved = resolveModule('./exact.css', fromFile, { cssSupport: true });
    expect(resolved).toBe(cssFile);
  });
  ...
  it('should handle CSS import when non-relative import returns null from resolveCssImport', () => {
    ...
    const resolved = resolveModule('some-package/styles.css', fromFile, { cssSupport: true });
    expect(resolved).toBeNull();
  });
  ...
  it('should handle external resolver code path (lines 220-225)', () => {
    ...
    process.env.NODE_PATH = path.join(__dirname, '..', '..', 'test-mocks', 'node_modules');
    require('node:module')._initPaths();

    // Clear module cache to ensure clean state
    const mockResolverPath = path.join(__dirname, '..', '..', 'test-mocks', 'node_modules', 'test-resolver-1');
    try {
  ...
  it('should handle CSS imports with non-relative paths (exercises line 109)', () => {
    ...
    const resolved = resolveModule('some-module', fromFile, { cssSupport: true });
  ...
  it('should handle error path in outer try-catch (lines 322-324)', () => {
    ...
    fs.writeFileSync(path.join(tsconfigDir, 'tsconfig.json'), '{"compilerOptions": {"paths": {"@/*": ["src/*"]}}}', 'utf-8');
