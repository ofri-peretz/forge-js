...
export function computeSCCsFromFile(
  ...
  while (filesToProcess.length > 0) {
    ...
    const imports = getFileImports(file, { workspaceRoot, barrelExports, cache, resolverSettings });
...
function tarjanStrongConnect(
  ...
  const { maxDepth, workspaceRoot, barrelExports, cache, resolverSettings } = options;
...
export function findAllCircularDependencies(
  ...
  const { maxDepth, reportAllCycles, workspaceRoot, barrelExports, cache, resolverSettings } =
    options;
  ...
  if (depth === 0) {
    ...
    computeSCCsFromFile(file, { maxDepth, workspaceRoot, barrelExports, cache, resolverSettings });
