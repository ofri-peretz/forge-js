...
describe('isNodeOfType', () => {
  ...
  it('should return false for undefined', () => {
    expect(isNodeOfType(undefined, AST_NODE_TYPES.VariableDeclaration)).toBe(false);
  });
...
describe('getFunctionName', () => {
  it('should return name for named function declaration', () => {
    const node = parseStatement('function foo() {}') as TSESTree.FunctionDeclaration;
    expect(getFunctionName(node)).toBe('foo');
  });

  it('should return name for named function expression', () => {
    const node = parseExpression('(function foo() {})') as TSESTree.FunctionExpression;
    expect(getFunctionName(node)).toBe('foo');
  });

  it('should return null for anonymous function', () => {
    const node = parseExpression('(function() {})') as TSESTree.FunctionExpression;
...
describe('isInsideNode', () => {
  it('should return true when node is inside specified parent type', () => {
    ...
    const innerFunction = (ast.body[0] as TSESTree.FunctionDeclaration)
      .body.body[0] as TSESTree.FunctionDeclaration;
    const ancestors: TSESTree.Node[] = [
      ast.body[0] as TSESTree.Node,
      (ast.body[0] as TSESTree.FunctionDeclaration).body,
    ];
    
    expect(isInsideNode(innerFunction, AST_NODE_TYPES.FunctionDeclaration, ancestors)).toBe(true);
  });
  ...
  it('should return false when node is not inside specified parent type', () => {
    ...
    expect(isInsideNode(node, AST_NODE_TYPES.FunctionDeclaration, ancestors)).toBe(false);
  });

  it('should return false for empty ancestors', () => {
    const node = parseStatement('const x = 5;');
    expect(isInsideNode(node, AST_NODE_TYPES.FunctionDeclaration, [])).toBe(false);
  });
});
...
describe('getAncestorOfType', () => {
  it('should return ancestor of specified type', () => {
    ...
    const result = getAncestorOfType(AST_NODE_TYPES.FunctionDeclaration, ancestors);
  ...
  it('should return null when no ancestor of type exists', () => {
    ...
    expect(getAncestorOfType(AST_NODE_TYPES.FunctionDeclaration, ancestors)).toBe(null);
  });

  it('should return null for empty ancestors', () => {
    expect(getAncestorOfType(AST_NODE_TYPES.FunctionDeclaration, [])).toBe(null);
  });
...
describe('extractFunctionSignature', () => {
  it('should extract signature for FunctionDeclaration with id', () => {
    const node = parseStatement('function test(a, b) { return a + b; }') as TSESTree.FunctionDeclaration;
  ...
  it('should extract signature for FunctionDeclaration without id', () => {
    ...
    const node = parseStatement('const fn = function(a, b) { return a + b; };') as TSESTree.VariableDeclaration;
  ...
  it('should extract signature for FunctionExpression', () => {
    const node = parseStatement('const fn = function(x, y) {};') as TSESTree.VariableDeclaration;
    const declarator = node.declarations[0];
    if (declarator.init && declarator.init.type === 'FunctionExpression') {
      const signature = extractFunctionSignature(declarator.init);
  ...
  it('should extract signature for ArrowFunctionExpression', () => {
    const node = parseStatement('const fn = (a, b) => a + b;') as TSESTree.VariableDeclaration;
    const declarator = node.declarations[0];
  ...
  it('should handle non-Identifier parameters (line 147)', () => {
    const node = parseStatement('function test(...args) { return args; }') as TSESTree.FunctionDeclaration;
    const signature = extractFunctionSignature(node);
  ...
  it('should handle object destructuring parameters', () => {
    const node = parseStatement('function test({ x, y }) { return x + y; }') as TSESTree.FunctionDeclaration;
    const signature = extractFunctionSignature(node);
