...
Prevent ambiguous link text like 'click here'. This rule is part of [`eslint-plugin-react-a11y`](https://www.npmjs.com/package/eslint-plugin-react-a11y) and provides LLM-optimized error messages with fix suggestions.
...
This rule helps ensure WCAG 2.4.4 compliance by enforcing: Prevent ambiguous link text like 'click here'
...
```tsx
// Violation of anchor-ambiguous-text
...
```tsx
// Compliant with anchor-ambiguous-text
// See rule source for specific examples
```
...
{
  rules: {
    'react-a11y/anchor-ambiguous-text': 'error'
  }
}
```
...
- **2.4.4 Link Purpose**: Prevent ambiguous link text like 'click here'
