...
Prevent positive tabindex values. This rule is part of [`eslint-plugin-react-a11y`](https://www.npmjs.com/package/eslint-plugin-react-a11y) and provides LLM-optimized error messages with fix suggestions.
...
This rule helps ensure WCAG 2.4.3 compliance by enforcing: Prevent positive tabindex values
...
```tsx
// Violation of tabindex-no-positive
...
```tsx
// Compliant with tabindex-no-positive
// See rule source for specific examples
```
...
- **2.4.3 Focus Order**: Prevent positive tabindex values
