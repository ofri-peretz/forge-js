...
Require labels on form controls. This rule is part of [`eslint-plugin-react-a11y`](https://www.npmjs.com/package/eslint-plugin-react-a11y) and provides LLM-optimized error messages with fix suggestions.
...
This rule helps ensure WCAG 1.3.1 compliance by enforcing: Require labels on form controls
...
```tsx
// Violation of control-has-associated-label
...
```tsx
// Compliant with control-has-associated-label
// See rule source for specific examples
```
...
- **1.3.1 Info and Relationships**: Require labels on form controls
