...
Prevent redundant words in alt text. This rule is part of [`eslint-plugin-react-a11y`](https://www.npmjs.com/package/eslint-plugin-react-a11y) and provides LLM-optimized error messages with fix suggestions.
...
This rule helps ensure WCAG 1.1.1 compliance by enforcing: Prevent redundant words in alt text
...
```tsx
// Violation of img-redundant-alt
...
```tsx
// Compliant with img-redundant-alt
// See rule source for specific examples
```
...
- **1.1.1 Non-text Content**: Prevent redundant words in alt text
