...
Prevent tabindex on non-interactive elements. This rule is part of [`eslint-plugin-react-a11y`](https://www.npmjs.com/package/eslint-plugin-react-a11y) and provides LLM-optimized error messages with fix suggestions.
...
This rule helps ensure WCAG 2.4.3 compliance by enforcing: Prevent tabindex on non-interactive elements
...
```tsx
// Violation of no-noninteractive-tabindex
...
```tsx
// Compliant with no-noninteractive-tabindex
// See rule source for specific examples
```
...
{
  rules: {
    'react-a11y/no-noninteractive-tabindex': 'error'
  }
}
```
...
- **2.4.3 Focus Order**: Prevent tabindex on non-interactive elements
