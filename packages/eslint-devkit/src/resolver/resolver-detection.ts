 ...
 */
export function detectResolvers(
  ...
  results.push({
    ...
    reason: viteConfig
      ? `Found ${viteConfig.split('/').pop()}`
      : 'No vite config found',
  });

  // Rollup resolver
  const rollupConfig =
    findFileUpward('rollup.config.js', workspaceRoot) ||
      ...
      ([ruleName, ruleConfig]) => {
        if (ruleName.startsWith('import/')) {
          ...
          if (ruleMappings[ruleName]) {
            ...
            suggestions.push(
              `Migrated ${ruleName} → ${ruleMappings[ruleName]}`,
            );
          } else {
            newRules[ruleName] = ruleConfig;
            warnings.push(
              `Rule ${ruleName} not automatically migrated - may need manual adjustment`,
            );
          }
        } else {
          newRules[ruleName] = ruleConfig;
