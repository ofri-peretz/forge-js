 ...
 * ```
 */
export function registerMessageTemplate(
  name: string,
...
export function getMessageTemplate(
  name: string,
): MessageTemplateConfig | undefined {
...
export function formatWithTemplate(options: TemplatedMessageOptions): string {
  ...
  const templateConfig =
    ...
    (options.templateName
      ? getMessageTemplate(options.templateName)
      : undefined);
  ...
  const owasp = (enriched as EnterpriseMessageOptions).owasp;
  const cvss =
    (enriched as EnterpriseMessageOptions).cvss ??
    severityToCVSS(enriched.severity as Severity);
  const compliance = (enriched as EnterpriseMessageOptions).compliance ?? [];
  ...
  if (templateConfig.jiraTemplate && options.jiraData) {
    jiraUrl = replacePlaceholders(templateConfig.jiraTemplate, {
      summary: encodeURIComponent(
        options.jiraData.summary ?? options.description,
      ),
  ...
  const owaspDisplay = owasp ? `OWASP:${owasp.split(':')[0]}` : '';

  // Build compliance display string (including custom compliance)
  const allCompliance = [
    ...compliance,
    ...(templateConfig.customCompliance ?? []),
  ];
  ...
  const line2Template =
    templateConfig.line2Format ?? '   Fix: {{fix}} | {{documentation}}';
...
export function toSARIF(options: EnterpriseMessageOptions): SARIFResult {
  ...
  return {
    ruleId:
      options.ruleId ||
      options.cwe ||
      options.issueName.replace(/\s+/g, '-').toLowerCase(),
    level: severityToSARIFLevel(enriched.severity as Severity),
    message: { text: enriched.description },
    properties: {
      'security-severity':
        enriched.cvss?.toString() ??
        severityToCVSS(enriched.severity as Severity).toString(),
      cwe: enriched.cwe,
