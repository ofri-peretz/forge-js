...
describe('getParserServices', () => {
  ...
  it('should return parser services from sourceCode.parserServices (ESLint 9)', () => {
    ...
    const result = getParserServices(
...
describe('getTypeOfNode', () => {
  ...
  it('should throw error when program is not available', () => {
    ...
    const servicesWithoutProgram: Partial<ParserServices> & {
      program?: undefined;
    } = {
      ...mockParserServices,
      program: undefined,
    ...
    expect(() =>
      getTypeOfNode(node, servicesWithoutProgram as ParserServices),
    ).toThrow('Program is not available');
  });
});
...
describe('getTypeArguments', () => {
  ...
  it('should use checker.getTypeArguments for array types', () => {
    const typeArgs = [{ flags: ts.TypeFlags.String } as ts.Type];
...
describe('typeMatchesPredicateRecursive', () => {
  ...
  it('should return true for intersection when all types match', () => {
    ...
    expect(typeMatchesPredicateRecursive(intersectionType, predicate)).toBe(
      true,
    );
  });
...
describe('getStringLiteralValues', () => {
  ...
  it('should return null when literalType.value is not a string (line 368)', () => {
    // Test case where literalType.value exists but is not a string
    const literalType = {
      flags: ts.TypeFlags.StringLiteral,
    ...
    expect(getStringLiteralValues(literalType)).toBeNull();
  });

  it('should return null when literalType.value is boolean', () => {
    const literalType = {
      flags: ts.TypeFlags.StringLiteral,
  ...
  it('should handle union type where some literal values are not strings (lines 379-383)', () => {
    // Create union where all types pass isStringLiteralType check (have StringLiteral flag)
    // but some have non-string values - this tests lines 379-382
    const stringLiteral1 = {
      flags: ts.TypeFlags.StringLiteral,
      value: 'name',
    } as unknown as ts.LiteralType;

    const stringLiteral2 = {
      flags: ts.TypeFlags.StringLiteral, // Has StringLiteral flag, so passes isStringLiteralType
      value: 123, // But value is number, not string - tests line 379 false branch
    } as unknown as ts.LiteralType;
    ...
    const unionType = {
      flags: 0,
      isUnion: () => true,
      types: [stringLiteral1, stringLiteral2, stringLiteral3],
    } as unknown as ts.Type;

    // Should return array with only string values, skipping the number one
    // Actually, looking at the code, if typeof literalType.value !== 'string', it just skips it
    // So it should return ['name', 'email']
    const result = getStringLiteralValues(unionType);
    expect(result).toEqual(['name', 'email']);
  });

  it('should handle union type where all literal values are non-strings (line 383)', () => {
    // All types are string literal types but all have non-string values
    const stringLiteral1 = {
      flags: ts.TypeFlags.StringLiteral,
      value: 123, // Number - tests line 379 false branch
    } as unknown as ts.LiteralType;

    const stringLiteral2 = {
      flags: ts.TypeFlags.StringLiteral,
      value: false, // Boolean - tests line 379 false branch
    } as unknown as ts.LiteralType;

    const unionType = {
      flags: 0,
      isUnion: () => true,
      types: [stringLiteral1, stringLiteral2],
    } as unknown as ts.Type;

    // Should return null because values.length === 0 (line 383)
    expect(getStringLiteralValues(unionType)).toBeNull();
  });
});
...
describe('isUnionOfSafeStringLiterals', () => {
  ...
  it('should use custom dangerous properties list', () => {
    ...
    expect(isUnionOfSafeStringLiterals(unionType, ['custom_dangerous'])).toBe(
      false,
    );
  });
});
