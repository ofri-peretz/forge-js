...
function createMockContext(
  options: {
    ...
    getScope?: () => {
      variables: Array<{ name: string; defs: Array<{ node: TSESTree.Node }> }>;
    };
    getCommentsBefore?: (
      node: TSESTree.Node,
...
describe('security-utils', () => {
  ...
  describe('SAFE_ANNOTATIONS', () => {
    ...
    it('should have exactly 9 annotations', () => {
      expect(SAFE_ANNOTATIONS).toHaveLength(9);
    });
  });
  ...
  describe('isSanitizationCall', () => {
    ...
    it('should return false for non-sanitization function calls', () => {
      const node = createMockCallExpression(
        createMockIdentifier('dangerousFunction'),
      );
    ...
    it('should support custom functions', () => {
      const node = createMockCallExpression(
        createMockIdentifier('myCustomSanitizer'),
      );
      expect(isSanitizationCall(node)).toBe(false);
    ...
    it('should return false for method name that is NOT a sanitization function', () => {
      ...
      const memberExpr = createMockMemberExpression(
        'unknownObject',
        'doSomething',
      );
      const node = createMockCallExpression(memberExpr);
    ...
    it('should handle nested member expressions (a.b.c)', () => {
      // Create nested member expression: a.b.c
      const innerMember: TSESTree.MemberExpression = {
        type: 'MemberExpression',
        object: createMockIdentifier('a'),
        property: createMockIdentifier('b'),
        computed: false,
        optional: false,
        range: [0, 3],
      ...
      const outerMember: TSESTree.MemberExpression = {
        type: 'MemberExpression',
        object: innerMember,
        property: createMockIdentifier('sanitize'),
        computed: false,
        optional: false,
        range: [0, 12],
      ...
      const node = createMockCallExpression(outerMember);
      // Should detect sanitize method name
      expect(isSanitizationCall(node)).toBe(true);
    });

    it('should handle getMemberExpressionPath when current ends as Identifier (line 288)', () => {
      // Test MemberExpression where object is Identifier (not nested)
      // This tests line 288: if (current.type === 'Identifier')
      const memberExpr = createMockMemberExpression('obj', 'method');
      const node = createMockCallExpression(memberExpr);

      // The path should be 'obj.method'
      expect(isSanitizationCall(node)).toBe(false); // method is not a sanitization function

      // But if method name is sanitize, it should work
      const sanitizeMember = createMockMemberExpression('obj', 'sanitize');
      const sanitizeNode = createMockCallExpression(sanitizeMember);
      expect(isSanitizationCall(sanitizeNode)).toBe(true);
    });

    it('should handle CallExpression when callee is not MemberExpression (tests line 255 condition)', () => {
      // When callee is Identifier, line 255 condition is false (callee.type !== 'MemberExpression')
      // This tests that the MemberExpression branch (line 255) is not taken
      const node = createMockCallExpression(createMockIdentifier('sanitize'));
      // Should hit Identifier branch (line 250), skipping MemberExpression branch (line 255)
      expect(isSanitizationCall(node)).toBe(true);

      // Test with callee that is neither Identifier nor MemberExpression
      // This ensures line 255 condition is evaluated (as false)
      const nodeWithOtherCallee: TSESTree.CallExpression = {
        type: 'CallExpression',
        callee: {
          type: 'Super',
          range: [0, 5],
      ...
      // Should return false - callee is not Identifier or MemberExpression
      // Line 255 condition will be evaluated as false
      expect(isSanitizationCall(nodeWithOtherCallee)).toBe(false);
    });

    it('should handle getMemberExpressionPath when current is not Identifier after loop (line 288 false branch)', () => {
      // Create MemberExpression where object is not Identifier (e.g., Literal or other type)
      const memberExpr: TSESTree.MemberExpression = {
        type: 'MemberExpression',
        object: {
          type: 'Literal',
        ...
        property: createMockIdentifier('method'),
        computed: false,
        optional: false,
        range: [0, 12],
        loc: { start: { line: 1, column: 0 }, end: { line: 1, column: 12 } },
      ...
      const node = createMockCallExpression(memberExpr);
      // This tests line 288 false branch (current.type !== 'Identifier')
      // The path building will skip adding the object name
      expect(isSanitizationCall(node)).toBe(false);
    });
  });
  ...
  describe('isSanitizedInput', () => {
    ...
    it('should return true for identifier assigned from sanitization call', () => {
      ...
      const initNode = createMockCallExpression(
        createMockIdentifier('sanitize'),
      );
      ...
      (
        context.sourceCode as unknown as { getScope: typeof context.getScope }
    ...
    it('should support custom sanitizer functions', () => {
      const node = createMockCallExpression(
        createMockIdentifier('myCustomClean'),
      );
      const context = createMockContext();
    ...
    it('should use context.getScope when sourceCode.getScope is unavailable (line 319)', () => {
      const node = createMockIdentifier('cleanInput');
      ...
      const context = {
        sourceCode: {
          getScope: undefined, // Not available
        },
        getScope: () => ({
          variables: [
            {
  ...
  describe('hasSafeAnnotation', () => {
    it('should return true for function with @safe JSDoc', () => {
      ...
      (innerNode as TSESTree.Node & { parent?: TSESTree.Node }).parent =
        funcNode;
      ...
      const context = createMockContext({
        getCommentsBefore: (node) => {
          if (node === funcNode) {
            return [
              { type: 'Block', value: '* @safe - validated by middleware ' },
            ];
          }
          return [];
        },
    ...
    it('should check nodeComments path with annotation in comment (line 397 true branch)', () => {
      const node = createMockIdentifier('value');
      // Node without parent to hit nodeComments path directly
      (node as TSESTree.Node & { parent?: TSESTree.Node }).parent = undefined;

      const context = createMockContext({
        getCommentsBefore: (n) => {
          if (n === node) {
            // Test line 397 true branch: comment contains annotation
            return [{ type: 'Block', value: '* @validated input' }];
          }
          return [];
        },
      });
    ...
    it('should check nodeComments path when annotation is not found (line 397 false branch)', () => {
      const node = createMockIdentifier('value');
      // Node without parent to hit nodeComments path directly
      (node as TSESTree.Node & { parent?: TSESTree.Node }).parent = undefined;
      ...
      const context = createMockContext({
        getCommentsBefore: (n) => {
          if (n === node) {
            // Test line 397 false branch: comment does NOT contain any annotation
            // Use a comment that definitely doesn't match any safe annotation
            return [{ type: 'Block', value: ' Just a regular comment ' }];
          }
          return [];
        },
      });

      // Should return false - no annotation found (line 397 condition is false)
    ...
    it('should support custom annotations', () => {
      ...
      expect(hasSafeAnnotation(innerNode, context)).toBe(false);
      expect(hasSafeAnnotation(innerNode, context, ['@my-custom-safe'])).toBe(
        true,
      );
    });
  ...
  describe('isOrmMethodCall', () => {
    ...
    it('should support custom patterns', () => {
      ...
      expect(isOrmMethodCall(node, context)).toBe(false);
      expect(isOrmMethodCall(node, context, ['customDb.customFind'])).toBe(
        true,
      );
    });
  ...
  describe('isParameterizedQuery', () => {
    it('should detect MySQL/SQLite style placeholders', () => {
      expect(isParameterizedQuery('SELECT * FROM users WHERE id = ?')).toBe(
        true,
      );
      expect(
        isParameterizedQuery('INSERT INTO users (name, email) VALUES (?, ?)'),
      ).toBe(true);
    });

    it('should detect PostgreSQL style placeholders', () => {
      expect(isParameterizedQuery('SELECT * FROM users WHERE id = $1')).toBe(
        true,
      );
      expect(
        isParameterizedQuery('SELECT * FROM users WHERE id = $1 AND name = $2'),
      ).toBe(true);
      expect(isParameterizedQuery('SELECT * FROM users WHERE id = $123')).toBe(
        true,
      );
    });

    it('should detect named parameter placeholders', () => {
      expect(isParameterizedQuery('SELECT * FROM users WHERE id = :id')).toBe(
        true,
      );
      expect(
        isParameterizedQuery(
    ...
    it('should return false for non-parameterized queries', () => {
      ...
      expect(
        isParameterizedQuery("SELECT * FROM users WHERE name = 'John'"),
      ).toBe(false);
    });
  ...
  describe('isInputSafe', () => {
    ...
    it('should support custom options', () => {
      ...
      expect(isInputSafe(node, context)).toBe(false);
      expect(
        isInputSafe(node, context, { customSanitizers: ['myClean'] }),
      ).toBe(true);
    });
  });
  ...
  describe('createSafetyChecker', () => {
    ...
    it('should support custom sanitizers', () => {
      ...
      const node = createMockCallExpression(
        createMockIdentifier('myCustomSanitize'),
      );
      const context = createMockContext();
    ...
    it('should work with all options combined', () => {
      ...
      const node = createMockCallExpression(
        createMockIdentifier('customSanitize'),
      );
      const context = createMockContext();
  ...
  describe('getEffectiveSeverity', () => {
    ...
    it('should use pattern-based override when context matches', () => {
      ...
      expect(
        getEffectiveSeverity('HIGH', override, { pattern: 'user-input' }),
      ).toBe('CRITICAL');
      expect(
        getEffectiveSeverity('HIGH', override, { pattern: 'internal-api' }),
      ).toBe('MEDIUM');
    });
    ...
    it('should fall back to default when pattern not found', () => {
      ...
      expect(
        getEffectiveSeverity('HIGH', override, { pattern: 'unknown-pattern' }),
      ).toBe('HIGH');
    });
    ...
    it('should prioritize pattern over level', () => {
      ...
      expect(
        getEffectiveSeverity('MEDIUM', override, { pattern: 'user-input' }),
      ).toBe('CRITICAL');
      // When pattern doesn't match, use level
      expect(
        getEffectiveSeverity('MEDIUM', override, { pattern: 'other' }),
      ).toBe('LOW');
    });
  ...
  describe('formatComplianceTags', () => {
    ...
    it('should limit to 4 frameworks', () => {
      const result = formatComplianceTags([
        'SOC2',
        'HIPAA',
        'PCI-DSS',
        'GDPR',
  ...
  describe('buildTicketUrl', () => {
    it('should return undefined when no template', () => {
      ...
      expect(
        buildTicketUrl({} as ComplianceContext['ticketTemplate'], {
          summary: 'Test',
        }),
      ).toBeUndefined();
    });
  ...
  describe('getDocumentationUrl', () => {
    it('should return default URL when no context', () => {
      expect(getDocumentationUrl('https://default.com')).toBe(
        'https://default.com',
      );
      expect(getDocumentationUrl('https://default.com', undefined)).toBe(
        'https://default.com',
      );
    });
    ...
    it('should return default URL when context has no override', () => {
      ...
      expect(getDocumentationUrl('https://default.com', context)).toBe(
        'https://default.com',
      );
    });
    ...
    it('should return override URL when provided', () => {
      ...
      expect(getDocumentationUrl('https://default.com', context)).toBe(
        'https://wiki.company.com/security',
      );
    });
  });
  ...
  describe('enhanceMessageData', () => {
    ...
    it('should use baseData description when context.summary is undefined (line 868)', () => {
      const result = enhanceMessageData(
        ...
        {
          // summary is undefined, should fall back to baseData['description']
        },
      );

      expect(result['ticketUrl']).toContain('Fallback%20description');
    });

    it('should handle baseData without description field', () => {
      const result = enhanceMessageData(
        { otherField: 'value' }, // No description field
        {
          compliance: {
            ticketTemplate: {
              url: 'https://jira.com/create?summary={{summary}}',
      ...
      expect(result['ticketUrl']).toContain('summary=');
    });
