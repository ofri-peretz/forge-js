...
export default [mcp.configs.recommended, mcp.configs.security];
```
...
> This package (`eslint-plugin-mcp`) re-exports `@forge-js/eslint-plugin-llm-optimized` with an MCP-focused name for developers using ESLint's Model Context Protocol.

**All these packages are functionally identical:**

- `eslint-plugin-mcp` (this package - MCP-focused)
