 ...
 * 1. Tarjan's SCC algorithm for O(V+E) guaranteed complete cycle detection
 * 2. Global SCC cache shared across all files in a lint run
 * 3. Pre-compiled regex patterns for import extraction
 * 4. Hash-based cache invalidation for incremental analysis
 *
 * Unlike eslint-plugin-import's no-cycle rule which can miss cycles at depth,
 * this implementation uses Strongly Connected Components (SCC) analysis to
 * guarantee finding ALL cycles regardless of depth.
 *
 * @coverage
...
const KNOWN_EXTERNAL_PACKAGES = new Set([
  ...
  'fs', 'path', 'url', 'http', 'https', 'crypto', 'util', 'os', 'child_process',
 ...
 */
export function createLRUCache<K, V>(maxSize: number = LRU_MAX_SIZE): LRUCache<K, V> {
...
export function resolveImportPath(
  ...
  if (isDefinitelyExternal(importPath)) {
    return null;
  }
  ...
  // Handle alias imports (@app/..., @/, etc.)
    ...
    // Check if it's a scoped package (not an alias) - e.g., @types/node, @babel/core
    // These start with @ but are external packages
    if (importPath.match(/^@[^/]+\/[^/]+/) && !importPath.match(/^@(app|src|lib)\//)) {
      // Likely an external scoped package
      result = null;
    } else {
      // Support @app/, @/, @src/, etc.
...
export function hasOnlyTypeImports(
  ...
  try {
    for (const file of cycle) {
      ...
      const hasRuntimeImport = !TYPE_IMPORT_REGEX.test(content) && 
        IMPORT_REGEX.test(content);
...
export function findAllCircularDependencies(
  ...
  if (depth === 0) {
    ...
    computeSCCsFromFile(file, { maxDepth, workspaceRoot, barrelExports, cache });
...
export function saveCacheToDisk(
  ...
  try {
    ...
    // Cache write failure is not critical - just continue without persistence
  }
}
...
export function loadCacheFromDisk(
  ...
  try {
    ...
    cache.sccIndex = new Map(Object.entries(data.sccIndex).map(
      ([k, v]) => [k, v as number]
    ));
